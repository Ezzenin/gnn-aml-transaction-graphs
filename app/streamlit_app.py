...
транзакции с подсветкой подозрительных по модели рёбер. Это демонстрационная
визуализация со структурными признаками окрестности (fan-in/out, реципрокность,
размеченный паттерн, если есть) — не полноценная pattern-mining система.
...
def main() -> None:
    ...
    st.caption("⚠️ Демо-сэмпл намеренно содержит ВСЕ laundering-рёбра выбранного "
               "test-среза + ограниченный фон легитимных — иначе при доле illicit "
               "~0.1% интерфейс почти всегда был бы пустым. Это витрина работы модели "
               "при дисбалансе, а не репрезентативная частота.")
    show = df.head(50).copy()
    st.dataframe(show, use_container_width=True, height=320)

    st.subheader("Разбор окрестности (структурные признаки)")
    pick = st.selectbox(
