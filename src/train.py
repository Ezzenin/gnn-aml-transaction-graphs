"""CLI: sanity-обучение мини-GCN на Elliptic (Задача 3.5, мостик к этапу 2).

Цель — убедиться, что PyG-конвейер обучается end-to-end на том же временно́м
сплите. НЕ оптимизировано и НЕ входит в приёмку этапа 1. На этапе 2 этот скрипт
расширяется до полноценного обучения GNN (GraphSAGE/GAT/GIN + адаптации, W&B).
...
def run(config: dict) -> dict:
    ...
    m_cfg = config.get("model", {}).get("params", {})
    model = GCN(
        in_channels=data.num_features,
        hidden_channels=int(m_cfg.get("hidden_channels", 64)),
        out_channels=2,
        dropout=float(m_cfg.get("dropout", 0.5)),
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_auc_pr = _eval_split(model, data, y_bin, val_mask)["auc_pr"]
        if val_auc_pr > best_val:
            best_val, best_state, bad = val_auc_pr, _clone_state(model), 0
        else:
            bad += 1
        if epoch % 20 == 0 or epoch == 1:
            print(f"epoch {epoch:3d} | loss {loss.item():.4f} | val AUC-PR {val_auc_pr:.4f}")
        if bad >= patience:
    ...
    test_m = _eval_split(model, data, y_bin, test_mask, threshold=val_m["threshold"])
    ...
    save_json(
        {"config": config, "val_metrics": val_m, "test_metrics": test_m},
        os.path.join(out_dir, "elliptic_gcn_sanity_metrics.json"),
    )
...
def _eval_split(model, data, y_bin, mask, threshold=None) -> dict:
    model.eval()
    out = model(data.x, data.edge_index)
    prob = F.softmax(out, dim=1)[:, 1]  # вероятность позитивного класса
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sanity GCN на Elliptic")
